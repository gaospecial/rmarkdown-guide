...
````{verbatim}
```{asy, elevation, fig.cap=' 用渐近线制作的 3D 图形。'}
import graph3;
import grid3;
import palette;
settings.prc = false;
...
```{asy echo=FALSE, fig.align='center', fig.retina=1, fig.width=6, cache=TRUE, elevation, fig.cap=' 用渐近线制作的 3D 图形。', eval=eval_asy()}
import graph3;
import grid3;
import palette;
settings.prc = false;
...
````md
```{asy, sine-curve, fig.cap='将数据从 R 传递到渐近线以绘制图形', cache=TRUE, fig.width=6,fig.align='center',fig.retina=1}`r ''`
import graph;
...
```{asy sine-curve, echo=FALSE, fig.align='center', fig.retina=1, fig.width=6,fig.cap='将数据从 R 传递到渐近线以绘制图形', eval=eval_asy()}
import graph;
size(400,300,IgnoreAspect);
settings.prc = false;
